...
numB = 100;                                                                   % Number of boundary points (including endpoint)
...
fprintf('X and Y coordinates of boundary points:')
disp(XB);
disp(YB);
% Number of panels
...
fprintf('Midpoints of X and Y coordinates :');
disp(XC);
disp(YC);
fprintf('panel length :');
disp(S);
...
fprintf('angles phi and beta for each panel in order:');
disp(phi);
disp(beta);
...
fprintf('velocities at midpoint of each panel:')
disp(Vt);
...
% Analytical angles and velocities
analyticTheta = linspace(0,2*pi,200)';                                      % Analytical theta angles [rad]
analyticV    = 2*Vinf*sin(analyticTheta);                                  % Analytical velocity []
...
% FIGURE: velocity magnitude vs theta 
if (flagPlot(6) == 1)
    figure(6);                                                              % Create figure
    cla; hold on; grid on;                                                  % Get ready for plotting
    set(gcf,'Color','White');                                               % Set color to white
    set(gca,'FontSize',12);                                                 % Set font size
    pA = plot(analyticTheta,analyticV,'k-','LineWidth',3);                 % Plot analytical velocity
    pC = plot(beta,Vt,'ks','MarkerFaceColor','r','MarkerSize',10);          % Plot compute velocity
    xlabel('Angle [rad]');                                                  % Set X-label
    ylabel('Velocity');                                                           % Set Y-label
    xlim([0 2*pi]);                                                         % Set X-limits
    ylim([-3.5 1.5]);                                                       % Set Y-limits
    legend([pA,pC],{'Analytical','SPM'},'Location','S');                    % Add legend
end
